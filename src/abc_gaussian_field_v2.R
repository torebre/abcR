...
# Trying a different way of generating the covariance matrix
# all.coords <- matrix(NA, nrow = grid.length^2, ncol = 2)
# coord.count <- 1
# for (i in 1:grid.length) {
#   for (j in 1:grid.length) {
#       all.coords[coord.count, 1] <- i
#       all.coords[coord.count, 2] <- j
#       coord.count <- coord.count + 1
#     }
#   }
# 
# cov.mat.x.prior <- matrix(NA, nrow = grid.length^2, ncol = grid.length^2)
# sapply(1:grid.length^2, function(i) {
#   print(i)
#   sapply(1:ceiling(grid.length / 2), function(j) {
#     cov.mat.x.prior[i, j] = cov.mat.x.prior[j, i] <- CalculateCovariance(all.coords[i, 1], all.coords[j, 1], all.coords[i, 2], all.coords[j, 2])
#   })
# })
# 
# for(i in 0:(grid.length^2 - 1)) {
#   if(i %% 1000 == 0) {
#     print(paste('Line ', i))
#   }
#   for(j in 0:ceiling((grid.length^2) / 2)) {
#     cov.mat.x.prior[i, j] = cov.mat.x.prior[j, i] <- CalculateCovariance(all.coords[i, 1], all.coords[j, 1], all.coords[i, 2], all.coords[j, 2])
#   }
# }
